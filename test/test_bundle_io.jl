...
@testset "_BundleIO Module Tests" begin
    ...
    @testset "Basic save and load" begin
        ...
        # Test save
        saved_path = save_preprocess_bundle(bundle, test_path)
        @test saved_path == abspath(test_path)
        @test isfile(saved_path)
        
        # Test load
        loaded_bundle = load_preprocess_bundle(saved_path)
    ...
    @testset "Save with compression options" begin
        ...
        save_preprocess_bundle(bundle, compressed_path, compress=true)
        @test isfile(compressed_path)
        
        # Test without compression
        uncompressed_path = joinpath(test_dir, "uncompressed.jld2")
        save_preprocess_bundle(bundle, uncompressed_path, compress=false)
        @test isfile(uncompressed_path)
        
        # Both should load successfully
        loaded1 = load_preprocess_bundle(compressed_path)
        loaded2 = load_preprocess_bundle(uncompressed_path)
    ...
    @testset "Directory creation" begin
        ...
        saved_path = save_preprocess_bundle(bundle, nested_path)
        @test isfile(saved_path)
    ...
    @testset "Format validation" begin
        ...
        @test_nowarn save_preprocess_bundle(bundle, test_path, format=:jld2)
        
        # Invalid format should error
        @test_throws ErrorException save_preprocess_bundle(bundle, test_path, format=:invalid)
        @test_throws ErrorException load_preprocess_bundle(test_path, format=:invalid)
    end
    ...
    @testset "File existence validation" begin
        ...
        @test_throws ArgumentError load_preprocess_bundle(nonexistent_path)
    end
    ...
    @testset "Version handling" begin
        ...
        # Save bundle
        save_preprocess_bundle(bundle, test_path)
        ...
        @test_nowarn load_preprocess_bundle(test_path)
    end
    ...
    @testset "Round-trip consistency" begin
        ...
        original_bundle = assemble_bundle(tokens, offsets, vocab, cfg)
        test_path = joinpath(test_dir, "roundtrip.jld2")
        
        # Save and load
        save_preprocess_bundle(original_bundle, test_path)
        loaded_bundle = load_preprocess_bundle(test_path)
    ...
    @testset "Path handling" begin
        ...
        saved_path = save_preprocess_bundle(bundle, relative_path)
        @test isabspath(saved_path)
        @test isfile(saved_path)
        
        # Should be able to load with both relative and absolute paths
        @test_nowarn load_preprocess_bundle(relative_path)
        @test_nowarn load_preprocess_bundle(saved_path)
