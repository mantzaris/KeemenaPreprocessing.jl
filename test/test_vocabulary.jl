...
const _TOKENS      = ["hello", "world", "hello", "test", "world", "hello"]
const _BYTES       = UInt8.('A':'C')           # ['A','B','C'] for the byte-path test
const _SPECIALS    = Dict(:unk => "<UNK>", :pad => "<PAD>")  # mirrors library default


@testset "build_vocabulary - specials (default cfg)" begin
    cfg   = PreprocessConfiguration()          # record_sentence_offsets == true
    vocab = build_vocabulary(_TOKENS; cfg)

    # 1.  Presence: first four entries must be the four specials,
    #     irrespective of order in the vector.
    @test Set(vocab.id_to_token_strings[1:4]) ==
          Set(["<UNK>", "<PAD>", "<BOS>", "<EOS>"])

    # 2.  Alphabetical ordering ->  ids(bos) < ids(eos) < ids(pad) < ids(unk)
...
@testset "build_vocabulary - specials (no sentence offsets)" begin
    cfg   = PreprocessConfiguration(record_sentence_offsets = false)
    vocab = build_vocabulary(_TOKENS; cfg)     # only :unk & :pad expected
...
@testset "build_vocabulary - minimum_token_frequency" begin
    cfg   = PreprocessConfiguration(minimum_token_frequency = 2,    # filter singletons
...
@testset "build_vocabulary _ deterministic ordering" begin
    ...
    vocab = build_vocabulary(tkns; cfg)
...
@testset "build_vocabulary - byte tokens overload" begin
    cfg   = PreprocessConfiguration(tokenizer_name = :byte,
                                    ...
                                    record_sentence_offsets = false)
    vocab = build_vocabulary(_BYTES; cfg)
...
@testset "build_vocabulary - empty token list" begin
    cfg   = PreprocessConfiguration(record_sentence_offsets = false)
    vocab = build_vocabulary(String[]; cfg)
