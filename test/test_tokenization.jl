...
const _DOC_SIMPLE  = ["Hello world.  Bye!"]                    # 1 doc
const _DOCS_MULTI  = ["One.\n\nTwo sentences here.",
                      "New doc—three sentences. OK?"]          # 2 docs
const _ASCII_PAR   = "A  B\n\nC"                               #splitter demo
const _UNICODE_TXT = "Café crème 😊."
...
@testset "tokenize_and_segment – whitespace defaults" begin
    cfg = PreprocessConfiguration()    # defaults: whitespace tok + all major offsets
    tokens, offs = tokenize_and_segment(_DOC_SIMPLE, cfg)
    ...
    # 1 document ⇒ doc offsets length == 2 (start+sentinel)
    @test length(offs[:document]) == 2
    # 2 sentences ⇒ sent offsets length == 3
...
@testset "tokenize_and_segment - paragraph split" begin
    cfg = PreprocessConfiguration(record_paragraph_offsets = true)
    tokens, offs = tokenize_and_segment([_ASCII_PAR], cfg)
...
@testset "tokenize_and_segment - unicode tokenizer" begin
    ...
    tokens, offs = tokenize_and_segment([_UNICODE_TXT], cfg)
...
@testset "tokenize_and_segment - char tokenizer + char offsets" begin
    cfg = PreprocessConfiguration(tokenizer_name = :char,
                                  record_character_offsets = true,
                                  record_word_offsets      = false,
                                  record_sentence_offsets  = false,
                                  record_document_offsets  = false)
    tokens, offs = tokenize_and_segment(["ab"], cfg)
...
@testset "tokenize_and_segment - byte tokenizer + byte offsets" begin
    cfg = PreprocessConfiguration(tokenizer_name = :byte,
                                  ...
                                  record_sentence_offsets   = false,
                                  record_document_offsets   = false)
    tokens, offs = tokenize_and_segment(["ABC"], cfg)
...
@testset "tokenize_and_segment - empty token filtering" begin
    cfg = PreprocessConfiguration(preserve_empty_tokens = false)
    tokens, _ = tokenize_and_segment(["a  b   c"], cfg)
...
@testset "tokenize_and_segment - invalid char-offset request" begin
    cfg = PreprocessConfiguration(tokenizer_name = :whitespace,
                                  record_character_offsets = true)
    @test_throws ErrorException tokenize_and_segment(["abc"], cfg)
end
