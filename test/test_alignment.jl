...
@testset "alignment functions" begin
    cm_bw = KeemenaPreprocessing._Alignment.alignment_byte_to_word(BYTE_CORP, WORD_CORP)
    @test cm_bw.source_level      == :byte
    @test cm_bw.destination_level == :word
    @test cm_bw.alignment         == [1, 2, 3]  

    cm_cw = KeemenaPreprocessing._Alignment.alignment_char_to_word(CHAR_CORP, WORD_CORP)
    @test cm_cw.alignment == [1, 2, 3]         

    cm_bc = KeemenaPreprocessing._Alignment.alignment_byte_to_char(BYTE_CORP, CHAR_CORP)
    @test cm_bc.alignment == [1, 2, 3]         

    wrong_word = _corpus(1, word_offs = [1, 2])
    @test_throws ArgumentError KeemenaPreprocessing._Alignment.alignment_byte_to_word(BYTE_CORP, wrong_word)
end
...
@testset "build_alignments!" begin
    ...
    KeemenaPreprocessing._Alignment.build_alignments!(bund)         # populate
