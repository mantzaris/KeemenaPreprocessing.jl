...
Self-contained pairing of a `Corpus` and its companion
`Vocabulary`.  A `LevelBundle` represents **one segmentation level**
...
Shorthand outer constructor that builds a `CrossMap` while **materialising
...
inside a `Corpus` where the start-offset vector for that level is
stored.
...
generated by `KeemenaPreprocessing` into a single `PreprocessBundle`.
...
|----------|------|-------------|
| `corpus` | `Corpus` | The corpus whose offsets are to be validated. |
